...
exports.prepopulateYear = prepopulateYear;
...
async function initDatabase(userDataPath) {
    ...
    const dbDir = path_1.default.join(userDataPath, 'ProTrack');
    if (!fs_1.default.existsSync(dbDir)) {
        fs_1.default.mkdirSync(dbDir, { recursive: true });
    }
    dbFilePath = path_1.default.join(dbDir, 'protrack.db');
    // Load WASM file
    // In development, the WASM file is in node_modules/sql.js/dist/
    // In production, we'll want to locate it. We'll handle both.
    let wasmPath = path_1.default.join(__dirname, '..', 'node_modules', 'sql.js', 'dist', 'sql-wasm.wasm');
    if (!fs_1.default.existsSync(wasmPath)) {
        // If running from dist-electron in production
        wasmPath = path_1.default.join(__dirname, 'sql-wasm.wasm');
    }
    if (!fs_1.default.existsSync(wasmPath)) {
        // Fallback search in parents
        wasmPath = path_1.default.resolve(__dirname, '../../node_modules/sql.js/dist/sql-wasm.wasm');
    }
...
function prepopulateYear(db, year) {
